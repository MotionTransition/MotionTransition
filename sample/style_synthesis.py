...
from data_loaders.humanml.utils.plotting import plot_conditional_samples, plot_style_samples
import json
import torch.nn.functional as F
from utils.motion_graph import dual_motion_transition, blend_motion, dual_blend_motion
...
def main():
    args = cond_synt_args()
    fixseed(args.seed)
    args.model_path = "./save/7315fxrn/model000100000.pt"
    ...
    max_frames = 196 if args.dataset in ['kit', 'humanml', 'style100', 'permo'] else (200 if args.dataset == 'trajectories' else 60)
    ...
    if 'motion_s' in model_kwargs["y"]:
        motion_s_list = []
        for t in model_kwargs["y"]['motion_s']:
            t = t.float().unsqueeze(1).permute(2, 1, 0)
            current_len = t.shape[-1]
            if current_len >= max_frames:
                motion_s_list.append(t[:, :, :max_frames])
            else:
                pad_size = max_frames - current_len
                processed_tensor = F.pad(t, (0, pad_size), value=0.0)
                motion_s_list.append(processed_tensor)
        motion_s = (torch.stack(motion_s_list, dim=0)).to(dist_util.dev())
    style_motions = motion_s
    ...
    for i in range(len(model_kwargs['y']['motion_s_name'])):
        all_style_name.append(model_kwargs['y']['motion_s_name'][i])
    for i in range(len(model_kwargs['y']['style_name'])):
        input_styles.append(model_kwargs['y']['style_name'][i])
    ...
    for rep_i in range(args.num_repetitions):
        ...
        model_kwargs['y']['nosty'] = False
        print("sample with style...")
        ...
        # model_kwargs['y']['nosty'] = True
        # print("sample without style...")
        # sample_nosty = sample_fn(
        #         model,
        #         (args.batch_size, model.njoints, model.nfeats, max_frames),
        #         clip_denoised=False,
        #         model_kwargs=model_kwargs,
        ...
        # blend
        # sample = blend_motion(sample_nosty, sample_sty, model_kwargs['y']['lengths'])
        # sample = dual_blend_motion(sample_nosty, sample_sty, model_kwargs['y']['lengths'])
        # sample = dual_motion_transition(sample_nosty, sample_sty, model_kwargs['y']['lengths'], model_kwargs['y']['lengths'])
    ...
    if model.data_rep == 'hml_vec':
        ...
        style_motions = style_motions.cpu().permute(0, 2, 3, 1)
        style_motions = data.dataset.t2m_dataset.inv_transform(style_motions).float()
        style_motions = recover_from_ric(data=style_motions, joints_num=n_joints, abs_3d=args.abs_3d)
        style_motions = style_motions.view(-1, *style_motions.shape[2:]).permute(0, 2, 3, 1)
        style_motions = style_motions.cpu().numpy()
    ...
    if args.dataset == 'humanml' or args.dataset == 'style100' or args.dataset == 'permo':
        plot_style_samples(motion=all_motions,
                                 lengths=all_lengths,
                                 texts=all_text,
                                 ...
                                 style_motion=style_motions,
                                 style_motion_name=all_style_name,
...
def load_dataset(args, max_frames, split='test'):
    conf = DatasetConfig(
        ...
        split=split,
        hml_mode='test',  # in train mode, you get both text and motion.
