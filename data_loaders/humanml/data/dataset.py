...
class STYLE100Dataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        min_motion_len = 40 if self.opt.dataset_name == 't2m' else 50
        ...
        for name in tqdm(id_list):
            try:
                ...
                style_dict[style].append(motion)
                new_name_list.append(name)
                if np.isnan(motion).any():
                    print(name, "motion has NaN!")
                ...
                # start_idx = 0
                # segment_idx = 0
                # while start_idx < total_frames - min_motion_len:
                #     segment_length = random.randint(min_motion_len, min(self.max_motion_length, total_frames - start_idx))
                #     end_idx = start_idx + segment_length

                #     if end_idx > total_frames:
                #         segment_length = end_idx - start_idx
                #     if segment_length < min_motion_len:
                #         break
                #     segment = motion[start_idx:end_idx]
                #     segment_name = f"{name}_{segment_idx:03d}"
                #     data_dict[segment_name] = {
                #         'motion': segment,
                #         'length': segment_length,
                #         'style': style
                #     }
                #     style_dict[style].append(segment)
                #     new_name_list.append(segment_name)
                #     if np.isnan(segment).any():
                #         print(segment_name, "motion has NaN!")
                ...
                pass
        print("total %d motions" % len(data_dict))

        name_list, length_list, tmp_style_list = zip(
            *sorted(zip(new_name_list, length_list, new_style_list), key=lambda x: x[1]))
        ...
        self.data_dict = data_dict
        self.name_list = name_list
        self.style_list = tmp_style_list
        self.reset_max_len(self.max_length)
    ...
    def __getitem__(self, item):
        ...
        motion_style_name = data['style']
        candidate_keys = [key for key in self.style_dict.keys() if key != motion_style_name]
        motion_s_name = random.choice(candidate_keys)
        motion_s = torch.tensor(random.choice(self.style_dict[motion_s_name]))
        motion_t = torch.tensor(random.choice(self.style_dict[motion_style_name]))
        ...
        std, mean = self.get_std_mean()
        motion = (motion - mean) / std
        motion_s = (motion_s - mean) / std
        motion_t = (motion_t - mean) / std
        ...
        # print(tokens)
        if self.mode == 'test':
            return motion, m_length, motion_style_name, motion_s, motion_s_name
        return motion, m_length, motion_style_name, motion_t, motion_s, motion_s_name
...
class PerMoDataset(data.Dataset):
    """
    使用reference style训练/生成PerMo
    """
    def __init__(self,
                 opt,
                 mean,
        ...
        print("style_num: ", len(style_dict.keys()))
        print("style_list: ", style_dict.keys())
        print("total %d motions" % len(data_dict))
...
class PerMoOnlyText(data.Dataset):
    """
    Args:
        std_multiplier: multiply the std by this value; maybe useful for diffusion models by keeping the range of data managable
    """
    ...
    def __getitem__(self, item):
        idx = self.pointer + item
        data = self.data_dict[self.name_list[idx]]
        motion, m_length, caption = data['motion'], data['length'], data[
            'text']
...
class STYLE100(data.Dataset):
    def __init__(self,
                 mode,
                 datapath='./dataset/style100_opt.txt',
                 split="train",
                 use_abs3d=False,
                 traject_only=False,
                 use_random_projection=False,
        ...
        if self.absolute_3d:
            ...
            #     self.mean = np.load(pjoin(opt.data_root, 'Mean.npy'))
            #     self.std = np.load(pjoin(opt.data_root, 'Std.npy'))
            elif mode in ['train', 'eval', 'text_only', 'test']:
                '''
                The 'eval' is here because we want inv_transform to work the same way at inference for model with abs3d,
                regradless of which dataset is loaded.
                '''
            ...
            self.mean_gt = np.load(
                pjoin(opt.meta_dir, f't2m_mean.npy'))
            self.std_gt = np.load(
                pjoin(opt.meta_dir, f't2m_std.npy'))
            self.mean_rel = np.load(pjoin(opt.data_root, 'Mean.npy'))
            self.std_rel = np.load(pjoin(opt.data_root, 'Std.npy'))
            self.mean_abs = np.load(pjoin(opt.data_root, 'Mean_abs_3d.npy'))
            self.std_abs = np.load(pjoin(opt.data_root, 'Std_abs_3d.npy'))
        ...
        if mode == 'text_only':
            ...
            print(f't2m dataset drop redundant information: {self.drop_redundant}')
            self.t2m_dataset = STYLE100Dataset(
                self.opt,
                self.mean,
                self.std,
                self.split_file,
...
class PerMo(data.Dataset):
    def __init__(self,
                 mode,
                 datapath='./dataset/permo_opt.txt',
                 split="train",
                 use_abs3d=False,
                 traject_only=False,
                 use_random_projection=False,
        ...
        self.split_file = pjoin(opt.data_root, f'{split}.txt')
        if mode == 'text_only':
            # 仅用text，不用reference style生成
            assert self.random_proj_scale == 10, 'mode text only support only random projection scale 10'
            print(
                f't2m dataset aug: {self.augment_type} std_scale_shift: {self.std_scale_shift}'
            )
            print(f't2m dataset drop redundant information: {self.drop_redundant}')
            self.t2m_dataset = PerMoOnlyText(
                self.opt,
                self.mean,
                self.std,
                self.split_file,
            ...
            #仅用reference style，不用text生成
            print(
                f't2m dataset aug: {self.augment_type} std_scale_shift: {self.std_scale_shift}'
            )
            print(f't2m dataset drop redundant information: {self.drop_redundant}')
            self.t2m_dataset = PerMoDataset(
                self.opt,
                self.mean,
                self.std,
                self.split_file,
