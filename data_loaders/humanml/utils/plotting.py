...
def plot_style_samples(motion, lengths, texts, observed_motion, observed_mask, num_samples, num_repetitions, out_path, edit_mode='benchmark_sparse', stop_imputation_at=0, style_motion=None, style_motion_name=None, input_styles = None):
    '''
    Used to plot samples during conditionally keyframed training.
    Arguments:
        motion {torch.Tensor} -- sampled batch of motions (nreps, nsamples, 22, 3, nframes)
    ...
    for sample_i in range(num_samples):
        ...
        if style_motion is not None:
            # caption = style_motion_name[sample_i]
            caption = style_motion_name[sample_i]
            length = lengths[0, sample_i]
            gt_motion = style_motion[sample_i].transpose(2, 0, 1)[:length]
            save_file = 'style_motion{:02d}.mp4'.format(sample_i)
            animation_save_path = os.path.join(out_path, save_file)
            rep_files.append(animation_save_path)
            print(f'[({sample_i}) "{caption}" | -> {save_file}]')
            plot_3d_motion(animation_save_path, skeleton, gt_motion, title=caption,
        ...
        all_rep_save_file = os.path.join(out_path, 'sample{:02d}.mp4'.format(sample_i))
        ffmpeg_rep_files = [f' -i {f} ' for f in rep_files]
        hstack_args = f' -filter_complex hstack=inputs={num_repetitions+2}'
        ffmpeg_rep_cmd = f'ffmpeg -y -loglevel warning ' + ''.join(ffmpeg_rep_files) + f'{hstack_args} {all_rep_save_file}'
        os.system(ffmpeg_rep_cmd)
        print(f'[({sample_i}) "{caption}" | all repetitions | -> {all_rep_save_file}]')
