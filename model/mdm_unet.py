...
class MDM_UNET(nn.Module):
    ...
    def __init__(self,
        ...
        if self.keyframe_conditioned:
            if self.dataset == 'humanml' and not self.xz_only:
                ...
                added_channels = 764
            elif self.dataset == 'style100' and not self.xz_only:
                added_channels = 263
            elif self.dataset == 'permo' and not self.xz_only:
                added_channels = 263
        else:
            added_channels = 0
        ...
        if self.cond_mode == 'style':
            # 使用风格作为条件时
            self.style_encoder = StyleClassification()
    ...
    def forward(self, x, timesteps, y=None, obs_x0=None, obs_mask=None):
        ...
        if self.keyframe_conditioned:
            assert self.dataset in ['humanml', 'amass', 'style100', 'permo']
            x = obs_x0 * obs_mask + x * (~obs_mask)
            x = torch.cat([x, obs_mask], dim=1)
        if y['sample'] == True:
            # 如果是采样
            return self.forward_sample(x, timesteps, y)
        return self.forward_core(x, timesteps, y)

    def forward_sample(self, x, timesteps, y=None):
        """
        Args:
            x: [batch_size, njoints, nfeats, max_frames], denoted x_t in the paper
            timesteps: [batch_size] (int)
        ...
        if 'style' in self.cond_mode:
            sty_mask = y.get('nosty', False)
            if sty_mask == False:
                if 'motion_s' in y:
                    motion_s_list = []
                    motion_s_lengths = []
                    ...
                    enc_style_s = self.style_encoder(motion_s, motion_s_lengths_torch)
                    emb += enc_style_s
        ...
        if self.keyframe_conditioned:
            njoints = 263 if self.dataset == 'humanml' or self.dataset == 'style100' or self.dataset == 'permo' else 764
    ...
    def forward_core(self, x, timesteps, y=None):
        ...
        if 'style' in self.cond_mode:
            with torch.no_grad():
                # 切断 x 的梯度链，确保不影响 unet 的梯度
                x_detached = x.detach()  # 关键操作：分离计算图
                enc_style_output = self.style_encoder(x_detached, y['lengths'])*y['style_mask']  # 使用 detached 的 x
            if torch.isnan(x).any():
                print("x has nan!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            return x, enc_style, enc_style_output
        else:
            return x
