...
from utils.motion_graph import dual_motion_transition
from utils.guidance import apply_guidance
...
class ClassifierFreeSampleModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None, obs_x0=None, obs_mask=None, **kwargs):
        ...
        assert cond_mode in ['text', 'action', 'style', 'no_cond']
        y_uncond = deepcopy(y)
        y_uncond['nosty'] = True
        ...
        # if timesteps[0] > 500:
        #     out = apply_guidance(out, obs_x0, y)
        # out = apply_guidance(out, obs_x0, y)
        out_uncond = self.model(x, timesteps, y_uncond, obs_x0, obs_mask, **kwargs)
        return out_uncond + (0.9 * (out - out_uncond))
        return out
