...
class TrainLoop:
    def __init__(self, args: TrainingOptions, model: nn.Module,
        ...
        # 风格标签，用来训练风格编码器
        self.selected_styles = ['Angry', 'Cold', 'Armaching', 'Ballerina', 'Childish', 'Crowded', 'Drunken', 'Old', 'Elegant', 'Exhausted', 'Fearful', 'Happy', 'Hulk', 'Headaching', 'Healthy', 'Legaching', 'Monkey', 'Muddyfloor', 'Neutral', 'Ninja', 'Penguin', 'Robot', 'Sad', 'Shy', 'Silly', 'Strained', 'Surprising', 'SWAT', 'Teenage', 'Textnecked', 'Unpleasantfloor', 'Uppity', 'Waiter', 'Zombie']
        self.name_to_id = {name: idx for idx, name in enumerate(self.selected_styles)}
        # self.parent_map = {4:1, 5:2, 6:3, 7:4, 8:5, 9:6, 10:7, 11:8, 12:9, 13:9, 14:9, 15:12, 16:13, 17:14, 18:16, 19:17, 20:18, 21:19}
        # self.bones_length = {}
        # T_pose = self.reference_styles['Aeroplane'][0]
        # for key in self.parent_map:
        #     child_idx = key*3+1
        #     parent_idx = self.parent_map[key]*3+1
        #     child = T_pose[child_idx:child_idx+3]
        #     parent = T_pose[parent_idx:parent_idx+3]
        #     dist = np.linalg.norm(child-parent)
        #     self.bones_length[key] = dist
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in tqdm(self.data):
                ...
                if self.args.keyframe_conditioned:
                    ...
                    cond['obs_mask'] = get_keyframes_mask(data=motion, lengths=cond['y']['lengths'], edit_mode='inbetween') # [bs, n_joints=263, nfeats=1, n_frames=224]
                ...
                if self.args.arch.startswith('style'):
                    # 训练风格编码器，增加标签
                    batch_style_labels = (torch.tensor([self.name_to_id[name] for name in cond['y']['style_name']], dtype=torch.long)).to("cuda:0")
                    cond['y']['style_ids'] = batch_style_labels
                
                # sample为True时，会调用mdm_unet采样的forward
                cond['y'].update({'sample': False})
