...
class GaussianDiffusion:
    ...
    def __init__(self, conf: DiffusionConfig):
        ...
        self.l1_loss = lambda a, b: torch.abs(
            a - b
        )
    ...
    def masked_l2_weighted(self, a, b, mask, weights, time_weights, over_keyframes=False, obs_mask=None):
        """
        Args:
            a: bs, J, Jdim, seqlen
            b: bs, J, Jdim, seqlen
        ...
        if obs_mask is None:
            ...
            loss = loss * mask.float() + loss * obs_mask.float()
    ...
    def masked_l1_weighted(self, a, b, mask, weights, time_weights, over_keyframes=False):
        """
        Args:
            a: bs, J, Jdim, seqlen
            b: bs, J, Jdim, seqlen
    ...
    def masked_l1_keyFrame(self, a, b, time_weights, lengths,  over_keyframes=False):
        """
        Args:
            a: bs, root_joints, Jdim, seqlen
            b: bs, root_joints, Jdim, seqlen
            time_weights: bs, root_joints, Jdim, seqlen
        """
        loss = self.l1_loss(a, b)
        # time_weights = time_weights / time_weights.sum(dim=[3], keepdims=True)
        loss = loss * time_weights
        loss = loss.sum(dim=[1, 2, 3]).abs() # 对有效长度数据进行扩展 [64] -> [64, 2, 1]
        loss = loss / lengths # 对应位相除 [64, 2, 1] / [64, 2, 1]，结果取平均
        return loss
    ...
    def bll_loss(self, predict, parent_map, bones_length, mask):
        non_zero_elements = sum_flat(mask)
        # 提取所有子节点和父节点的索引
        child_indices = torch.tensor([key * 3 + 1 for key in parent_map.keys()])  # 子节点起始索引
        parent_indices = torch.tensor([parent_key * 3 + 1 for parent_key in parent_map.values()])  # 父节点起始索引

        # 提取子节点和父节点的 3D 位置
        child_positions = predict[:, child_indices[:, None] + torch.arange(3), 0, :]  # 形状为 (64, len(self.parent_map), 3, 224)
        parent_positions = predict[:, parent_indices[:, None] + torch.arange(3), 0, :]  # 形状为 (64, len(self.parent_map), 3, 224)

        # 计算 L2 距离
        distances = torch.norm(child_positions - parent_positions, dim=2)  # 形状为 (64, len(self.parent_map), 224)

        # 将字典转换为张量，顺序与 self.parent_map 中的子节点顺序一致
        bones_lengths_tensor = (torch.tensor([bones_length[key] for key in parent_map.keys()])).to('cuda:0')  # 形状为 (len(self.parent_map),)

        # 减去预计算长度并取绝对值
        result = (torch.abs(distances - bones_lengths_tensor[:, None])).to('cuda:0')  # 形状为 (64, len(self.parent_map), 224)

        result = torch.mean(result, dim=1)

        mask = mask.squeeze(2).squeeze(1)
        result = result*mask
        result = sum_flat(result)
        result = result/non_zero_elements
        return result
    ...
    def training_losses(self,
        ...
        style_mask = torch.bernoulli(
                torch.ones(bs, device=x_start.device) * style_mask_prob).view(
                    bs, 1)  # 1-> use null_cond, 0-> use real cond
        model_kwargs['y']['style_mask'] = (1.-style_mask)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [ # false
            ...
            if enc.zero_keyframe_loss: # false
            ...
            # 计算重建损失
            if 'motion_t' in model_kwargs['y'] and 'motion_s' in model_kwargs['y']:
                loss_con1 = self.masked_l2_weighted(
                    target, model_output_x, mask,
                    weights=weights,
                    time_weights=time_weights)  # mean_flat(rot_mse)

                loss_con2 = self.masked_l2_weighted(
                    target, model_output_t, mask,
                    weights=weights,
                    time_weights=time_weights)  # mean_flat(rot_mse)
            ...
            if 'motion_t' in model_kwargs['y'] and 'motion_s' in model_kwargs['y']:
                # 计算风格损失
                loss_sty1 = torch.abs(enc_style_s - enc_style_outs).mean(dim=1)
                loss_sty2 = torch.abs(enc_style_x - enc_style_outx).mean(dim=1)
                loss_sty3 = torch.abs(enc_style_t - enc_style_outt).mean(dim=1)
                terms["style_loss"] = ((loss_sty1+loss_sty2+loss_sty3)/3)*model_kwargs['y']['style_mask']
                if torch.isnan(terms["style_loss"]).any():
                    print("style_loss has nan!")

            if model_output2 is not None: # false
            ...
            if self.lambda_vel_rcxyz > 0.: # false
                assert self.conf.traj_extra_weight == 1
            ...
            terms["loss"] = terms.get("rot_mse", 0.) + terms.get('rot_mse2', 0.) + terms.get('vb', 0.) +\
            ...
            if self.conf.train_x0_as_eps: # false
